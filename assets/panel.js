...
function renderGlobalSettingsModal() {
  ...
  return `<div class="modal-backdrop" data-action="close-modal"><section class="modal settings-modal" id="settings-config" role="dialog" aria-modal="true" aria-labelledby="global-settings-title"><div class="modal-header"><div><p class="eyebrow">PLUGIN SETTINGS</p><h2 id="global-settings-title">全局设置</h2></div><button class="close-button" data-action="close-modal" aria-label="关闭">×</button></div><form id="global-settings-form" class="modal-form"><p class="hint">推荐：embedding 用硅基流动的 <code>BAAI/bge-m3</code>，LLM 用 <code>deepseek-v4-flash</code>。API key 只保存在本机插件数据目录。</p><p class="settings-section-title">Embedding</p><label>API Key<input name="embeddingApiKey" type="password" autocomplete="new-password" placeholder="${escapeHtml(embeddingKeyPlaceholder)}"></label><div class="settings-grid"><label>Base URL<input name="embeddingBaseUrl" value="${escapeHtml(embedding.baseUrl)}" placeholder="https://api.siliconflow.cn/v1" required></label><label>Model<input name="embeddingModel" value="${escapeHtml(embedding.model)}" placeholder="BAAI/bge-m3" required></label></div><p class="settings-section-title">LLM 抽取（图谱）</p><label class="check-line"><input name="llmSameAsEmbedding" type="checkbox" ${llm.sameAsEmbedding ? "checked" : ""}> 与 embedding 使用相同 API Key</label><label>API Key<input name="llmApiKey" type="password" autocomplete="new-password" placeholder="${escapeHtml(llmKeyPlaceholder)}"></label><div class="settings-grid"><label>Base URL<input name="llmBaseUrl" value="${escapeHtml(llm.baseUrl)}" placeholder="https://api.siliconflow.cn/v1" required></label><label>Model<input name="llmModel" value="${escapeHtml(llm.model)}" placeholder="deepseek-v4-flash" required></label></div><div class="modal-actions"><button class="outline-button" type="button" data-action="close-modal">取消</button><button class="primary-button" type="submit">保存全局设置</button></div></form></section></div>`;
}
